...
function seatsSummary() {
    var summaryElement = document.getElementById("seats_summary");
    if (!summaryElement) {
        return Promise.resolve();
    }
    summaryElement.innerHTML = '<span class="loading-indicator"><i class="fa fa-spinner fa-spin" aria-hidden="true"></i> Loading seats…</span>';

    var request;
    if (typeof window !== 'undefined' && typeof window.fetch === 'function') {
        request = fetch("/2017/NI/all-elected-d3.json", { cache: 'no-store' }).then(function(response) {
            if (!response.ok) {
                var error = new Error('Failed to fetch seat summary');
    ...
    return request.then(function(data) {
        ...
        _.forEach(result, function(value, key) {
            summaryElement.innerHTML += '<span class="' + key.replace(/\s+/g, "-") + '">&nbsp;&nbsp;&nbsp;</span>&nbsp;<span class="' + key.replace(/\s+/g, "-") + '_abbr ' + '"></span>' + ': ' + value + '&nbsp;';
        });
    }).catch(function(error) {
        console.error('Unable to load seat summary', error);
        summaryElement.innerHTML = 'Unable to load seat summary';
    });
}
