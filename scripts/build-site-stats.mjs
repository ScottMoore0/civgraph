...
// The graph is the ONE input this does not insist on.
//
// data/graph is generated by `npm run build:graph`, which is not part of `npm run build`,
// and it is gitignored -- so on a clean checkout it does not exist when this runs. Making
// it a hard error broke the Cloudflare Pages build while passing locally, where the graph
// happened to have been built by hand. Every other source here is either committed or
// generated earlier in the same chain.
//
// So the graph figure is skipped when the graph is absent rather than failing. It is an
// opt-in figure on one page; a page that claims it is still checked whenever it can be.
const graphManifest = optionalJson('data/graph/manifest.json');
...
// An optional figure whose source is absent is dropped, not zeroed: a stat that silently
// reads 0 is exactly the kind of plausible-looking wrong number this file exists to stop.
for (const key of Object.keys(stats)) {
  if (stats[key] === null || stats[key] === undefined) delete stats[key];
}
